...
const HearingImpairment = () => {
  ...
  const {
    data: hearingImpairmentSpecialData,
    error,
    isLoading,
    refetch,
  } = useQuery({
    ...
    queryFn: async () => {
      try {
        const { data: res } = await AxiosConfig.get(
          "/specialeducation/hearingImpairment/search",
          {
            params: {
      ...
      try {
        const { data } = await AxiosConfig.post(
          "/specialeducation/hearingImpairment",
          {
            ...
            educationService_id: specialData,
            id: hearingImpairmentSpecialData?._id,
          }
        );
  ...
  useEffect(() => {
    console.log(hearingImpairmentSpecialData);
    if (hearingImpairmentSpecialData && !isLoading) {
      console.log({ hearingImpairmentSpecialData });
      setFormData(hearingImpairmentSpecialData);
    } else if (!isLoading) {
      setFormData(initialData);
    }
  }, [hearingImpairmentSpecialData]);
  return (
...
export default HearingImpairment;
