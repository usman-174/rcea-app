...
const QafSchemeOfWork = () => {
  ...
  const shouldWeFetchSchemeOfWork =
    selectedGrade &&
    selectedSection &&
    currentSelectedTerm &&
  ...
  const {
    ...
    queryFn: async ({ queryKey }) => {
      ...
      const response = await AxiosConfig.get("qaf/schemeofwork/get", {
        params: {
  ...
  const createSchemeOfWork = useMutation({
    mutationFn: async (data) => {
      const response = await AxiosConfig.post("/qaf/schemeofwork/create", data);
      return response.data;
    },
    onSuccess: () => {
  ...
  const updateSchemeOfWork = useMutation({
    mutationFn: async (data) => {
      const response = await AxiosConfig.put(`/qaf/schemeofwork/update/${data?.id}`, data?.data);
      return response.data;
    },
    onSuccess: () => {
  ...
  const uploadSchemeOfWork = (e) => {
    ...
    const data = {
      ...
      term: currentSelectedTerm?.value,
      subject: selectedSubject?.value,
      // prescribedTextbook: selectedPrescribedTextbook?.value,
      prescribes_book: selectedPrescribedTextbook?.value,
      // numberOfPeriodsPerWeek: selectedNumberOfPeriodsPerWeek?.value,
  ...
  return (
    <div>
      <Toaster />
      <Container>
        <h2 className="pt-5 mb-4 pb-2">Qaf Scheme of Work</h2>
        {error && (
