...
const { selectedSchool } = useSelector((state) => state.school);
  ...
  const handleSave = () => {
    ...
    AxiosConfig.post("/phoSegscr/create", result)
      .then(() => {
  ...
  useEffect(() => {
    ...
    const fetchScreenPupils = async () => {
      try {
        const response = await AxiosConfig.post("/phoSegscr/all", {
          grade: selectedGrade.value,
          language: selectedLanguage.value,
          student_section: selectedSection.value,
        });
  ...
  return (
    <div>
      <Toaster />
      <Container>
        <h2 className="pt-5 mb-4 pb-2">Phonemic Segmentation Fluency</h2>
        {error && (
