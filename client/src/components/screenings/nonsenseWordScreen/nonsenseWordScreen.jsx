...
const { selectedSchool } = useSelector((state) => state.school);
  ...
  const handleSave = () => {
    ...
    AxiosConfig.post("/nonsenseWordscr/create", result)
      .then(() => {
  ...
  useEffect(() => {
    ...
    const fetchScreenPupils = async () => {
      try {
        const response = await AxiosConfig.post("/nonsenseWordscr/all", {
          grade: selectedGrade.value,
          language: selectedLanguage.value,
          student_section: selectedSection.value,
        });
  ...
  return (
    <div>
      <Toaster />
      <Container>
        <h2 className="pt-5 mb-4 pb-2">Nonsense Word Fluency</h2>
        {error && (
