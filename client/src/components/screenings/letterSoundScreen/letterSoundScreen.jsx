...
const { selectedSchool } = useSelector((state) => state.school);
  ...
  const handleSave = () => {
    ...
    AxiosConfig.post("/letterSoundscr/create", result)
      .then(() => {
  ...
  useEffect(() => {
    ...
    const fetchScreenPupils = async () => {
      try {
        const response = await AxiosConfig.post("/letterSoundscr/all", {
          grade: selectedGrade.value,
          language: selectedLanguage.value,
          student_section: selectedSection.value,
        });
        motorData = response.data;
        // setScreeningPupils(response.data);
      } catch (err) {
        console.log("error");
      }
    };
  ...
  return (
    <div>
      <Toaster />
      <Container>
        <h2 className="pt-5 mb-4 pb-2">Letter Sound Fluency</h2>
        {error && (
