...
import { sections,grades,language } from "../../../utils/globals";
import { addPropertiesToPupils } from "../settings/addProperties";
import { getColumns } from "../settings/getColumns";
import { getRows } from "../settings/getRows";
import { generateHeaderRow } from "../settings/headerRow";
import { Column_Row_Fields, HEADER_COLS, fields } from "./fields";
import { useSelector } from "react-redux";


const headerRow = generateHeaderRow(HEADER_COLS);
...
const { selectedSchool } = useSelector((state) => state.school);
  ...
  const handleSave = () => {
    ...
    AxiosConfig.post("/past/create", result)
      .then(() => {
  ...
  useEffect(() => {
    ...
    const fetchScreenPupils = async () => {
      try {
        const response = await AxiosConfig.post("/past/all", {
          grade: selectedGrade.value,
          language: selectedLanguage.value,
          student_section: selectedSection.value,
        });
    ...
    if (selectedGrade && selectedSection && selectedLanguage) {
  ...
  }, [selectedGrade, selectedSection, selectedLanguage, successSave,selectedSchool._id]);
  ...
  return (
    <div>
      <Toaster />
      <Container>
        <h2 className="pt-5 mb-4 pb-2">PAST</h2>
        {error && (
