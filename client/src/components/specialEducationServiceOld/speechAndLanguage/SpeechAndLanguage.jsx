...
const specialData = searchParams.get("specialData");
    ...
    const {
        data: speechAndLanguageSpecialData,
        error,
        isLoading,
        refetch
    } = useQuery({
        ...
        queryFn: async () => {
            try {
                const { data: res } = await AxiosConfig.get("/specialeducation/speechandlanguage/search", {
                    params: {
    ...
    const mutation = useMutation({
        onMutate: async () => {
            try {
                const { data } = await AxiosConfig.post("/specialeducation/speechandlanguage", {
                    ...formData,

                    educationService_id: specialData
                    , id: speechAndLanguageSpecialData?._id
                })
    ...
    useEffect(() => {
        console.log(speechAndLanguageSpecialData);
        if (speechAndLanguageSpecialData && !isLoading
        ) {
            console.log({ speechAndLanguageSpecialData });
            setFormData(speechAndLanguageSpecialData)
        } else if (!isLoading) {
            setFormData(initialData)
        }

    }, [speechAndLanguageSpecialData])
    return (
...
export default SpeechAndLanguageSpecial