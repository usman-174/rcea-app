...
const letterSoundScreening = require('../models/letterSoundScreening')
...
const Controller = {
  ...
  create: catchAsync(async (req, res) => {
    ...
    try {
      const updateOperations = data.map(async (item) => {
        ...
        const result = await letterSoundScreening.findOneAndUpdate(
          { pupil_id, language },
          item,
  ...
  //     const NewletterNamingScreening = await letterSoundScreening.insertMany(
  //       data
  //     )
  //     if (!NewletterNamingScreening) {
  //       return res
  //         .status(400)
  //         .json({ error: 'letterSoundScreening not created ' })
  //     } else {
    ...
    try {
      ...
      await letterSoundScreening
        ...
        .catch(() => {
          res
            .status(404)
            .send({ message: 'Not letterSoundScreening  with id ' + id })
        })
    } catch (error) {
      return res
        .status(400)
      ...
      letterSoundScreening
        ...
        .then((data) => {
          if (!data) {
            ...
            res.send({
              message: 'letterSoundScreening is deleted successfully!',
            })
          }
        })
        .catch((error) => {
          return res
            .status(400)
            .json({ error: `letterSoundScreening Dosent exist` })
        })
    } catch (error) {
      return res
        .status(400)
