...
const SpeechAndLanguageModel = require("../models/speechAndLanguage");
...
const controller = {
  getall: catchAsync(async (req, res) => {
    try {
      ...
      const filterScreen = await SpeechAndLanguageModel.find({ language })
        .populate({
          path: "pupil_id",
          match: {
  ...
  create: catchAsync(async (req, res) => {
    ...
    try {
      const updateOperations = data.map(async (item) => {
        ...
        const result = await SpeechAndLanguageModel.findOneAndUpdate(
          { pupil_id, language },
          item,
