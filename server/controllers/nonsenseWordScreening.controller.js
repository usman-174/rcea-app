...
const nonsenseWordScreening = require('../models/nonsenseWordScreening')
...
const Controller = {
  ...
  create: catchAsync(async (req, res) => {
    ...
    try {
      const updateOperations = data.map(async (item) => {
        ...
        const result = await nonsenseWordScreening.findOneAndUpdate(
          { pupil_id, language },
          item,
  ...
  //     const NewnonsenseWordScreening = await nonsenseWordScreening.insertMany(
  //       data
  //     )
  //     if (!NewnonsenseWordScreening) {
  //       return res
  //         .status(400)
  //         .json({ error: 'nonsenseWordScreening not created ' })
  //     } else {
  //       res.status(200).json(NewnonsenseWordScreening)
  //     }
  //   } catch (error) {
  //     res.status(400).json({ error: error.message })
  //   }
    ...
    try {
      ...
      await nonsenseWordScreening
        ...
        .catch(() => {
          res
            .status(404)
            .send({ message: 'Not nonsenseWordScreening  with id ' + id })
        })
    } catch (error) {
      return res
        .status(400)
      ...
      nonsenseWordScreening
        ...
        .then((data) => {
          if (!data) {
            ...
            res.send({
              message: 'nonsenseWordScreening is deleted successfully!',
            })
          }
        })
        .catch((error) => {
          return res
            .status(400)
            .json({ error: `nonsenseWordScreening Dosent exist` })
        })
    } catch (error) {
      return res
        .status(400)
