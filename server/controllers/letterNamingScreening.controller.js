...
const letterNamingScreening = require('../models/letterNamingScreening')
...
const Controller = {
  ...
  create: catchAsync(async (req, res) => {
    ...
    try {
      const updateOperations = data.map(async (item) => {
        ...
        const result = await letterNamingScreening.findOneAndUpdate(
          { pupil_id, language },
          item,
  ...
  //     const NewletterNamingScreening = await letterNamingScreening.insertMany(
  //       data
  //     )
  //     if (!NewletterNamingScreening) {
  //       return res
  //         .status(400)
  //         .json({ error: 'letterNamingScreening not created ' })
  //     } else {
  //       res.status(200).json(NewletterNamingScreening)
  //     }
  //   } catch (error) {
  //     res.status(400).json({ error: error.message })
  //   }
    ...
    try {
      ...
      await letterNamingScreening
        ...
        .catch(() => {
          res
            .status(404)
            .send({ message: 'Not letterNamingScreening  with id ' + id })
        })
    } catch (error) {
      return res
        .status(400)
      ...
      letterNamingScreening
        ...
        .then((data) => {
          if (!data) {
            ...
            res.send({
              message: 'letterNamingScreening is deleted successfully!',
            })
          }
        })
        .catch((error) => {
          return res
            .status(400)
            .json({ error: `letterNamingScreening Dosent exist` })
        })
    } catch (error) {
      return res
        .status(400)
