...
const PsychoMotorModel = require("../models/psychoMotorScreem");
...
const controller = {
  getall: catchAsync(async (req, res) => {
    try {
      ...
      const filterScreen = await PsychoMotorModel.find({ language })
        .populate({
          path: "pupil_id",
          match: {
  ...
  create: catchAsync(async (req, res) => {
    ...
    try {
      const updateOperations = data.map(async (item) => {
        ...
        const result = await PsychoMotorModel.findOneAndUpdate(
          { pupil_id, language },
          item,
