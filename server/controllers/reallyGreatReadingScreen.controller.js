...
const ReallyGreatReadingModel = require("../models/reallyGreatReadingScreen");
...
const controller = {
  getall: catchAsync(async (req, res) => {
    try {
      ...
      const filterScreen = await ReallyGreatReadingModel.find({ language })
        .populate({
          path: "pupil_id",
          match: {
  ...
  create: catchAsync(async (req, res) => {
    ...
    try {
      const updateOperations = data.map(async (item) => {
        ...
        const result = await ReallyGreatReadingModel.findOneAndUpdate(
          { pupil_id, language },
          item,
