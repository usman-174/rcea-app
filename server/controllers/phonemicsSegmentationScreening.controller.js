...
const phonemicsSegmentationScreening = require('../models/phonemicsSegmentationScreening')
...
const Controller = {
  getall: catchAsync(async (req, res) => {
    try {
      ...
      await phonemicsSegmentationScreening
  ...
  create: catchAsync(async (req, res) => {
    ...
    try {
      const updateOperations = data.map(async (item) => {
        ...
        const result = await phonemicsSegmentationScreening.findOneAndUpdate(
          { pupil_id, language },
          item,
  ...
  //     const NewphonemicsSegmentationScreening = await phonemicsSegmentationScreening.insertMany(
  //       data
  //     )
  //     if (!NewphonemicsSegmentationScreening) {
  //       return res
  //         .status(400)
  //         .json({ error: 'phonemicsSegmentationScreening not created ' })
  //     } else {
  //       res.status(200).json(NewphonemicsSegmentationScreening)
  //     }
  //   } catch (error) {
  //     res.status(400).json({ error: error.message })
  //   }
    ...
    try {
      ...
      await phonemicsSegmentationScreening
        ...
        .catch(() => {
          res
            .status(404)
            .send({ message: 'Not phonemicsSegmentationScreening  with id ' + id })
        })
    } catch (error) {
      return res
        .status(400)
      ...
      phonemicsSegmentationScreening
        ...
        .then((data) => {
          if (!data) {
            ...
            res.send({
              message: 'phonemicsSegmentationScreening is deleted successfully!',
            })
          }
        })
        .catch((error) => {
          return res
            .status(400)
            .json({ error: `phonemicsSegmentationScreening Dosent exist` })
        })
    } catch (error) {
      return res
        .status(400)
