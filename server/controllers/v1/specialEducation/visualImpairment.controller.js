...
const visualImpairmentSchema = require("../../../models/v1/specialEducationService/visualImpairment");

const Controller = {
  getAll: async (req, res) => {
    try {
      const data = await visualImpairmentSchema.find();
  ...
  createOrUpdate: async (req, res) => {
    ...
    try {
      ...
      if (id) {
        ...
        data = await visualImpairmentSchema.findByIdAndUpdate(id, req.body, {
          new: true,
        });
        if (!data) return res.status(404).json({ message: "Record not found" });
      } else {
        // If ID is not provided, create new entry
        const core = new visualImpairmentSchema(req.body);
  ...
  getById: async (req, res) => {
    try {
      const data = await visualImpairmentSchema.findById(req.params.id);
  ...
  deletAll: async (req, res) => {
    try {
      const data = await visualImpairmentSchema.deleteMany();
      return res.status(200).json(data);
    } catch (error) {
      return res.status(500).json({ message: error.message });
    }
  ...
  search: async (req, res) => {
    ...
    try {
      ...
      const data = await visualImpairmentSchema.findOne(query);
