...
const hearingImpairmentSchema = require("../../../models/v1/specialEducationService/hearingImpairment");

const Controller = {
  getAll: async (req, res) => {
    try {
      const data = await hearingImpairmentSchema.find();
  ...
  createOrUpdate: async (req, res) => {
    ...
    try {
      ...
      if (id) {
        ...
        data = await hearingImpairmentSchema.findByIdAndUpdate(id, req.body, {
          new: true,
        });
        if (!data) return res.status(404).json({ message: "Record not found" });
      } else {
        // If ID is not provided, create new entry
        const core = new hearingImpairmentSchema(req.body);
  ...
  getById: async (req, res) => {
    try {
      const data = await hearingImpairmentSchema.findById(req.params.id);
  ...
  deletAll: async (req, res) => {
    try {
      const data = await hearingImpairmentSchema.deleteMany();
      return res.status(200).json(data);
    } catch (error) {
      return res.status(500).json({ message: error.message });
    }
  ...
  search: async (req, res) => {
    ...
    try {
      ...
      const data = await hearingImpairmentSchema.findOne(query);
