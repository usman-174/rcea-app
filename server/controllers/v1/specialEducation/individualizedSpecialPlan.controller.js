...
const IndividualizedSpecialPlan = require("../../../models/v1/specialEducationService/individualizedSpecialPlan");

const Controller = {
  getAll: async (req, res) => {
    try {
      const data = await IndividualizedSpecialPlan.find();
  ...
  createOrUpdate: async (req, res) => {
    ...
    try {
      ...
      if (id) {
        ...
        data = await IndividualizedSpecialPlan.findByIdAndUpdate(
          id,
          req.body,
          {
            new: true,
        ...
        const core = new IndividualizedSpecialPlan(req.body);
  ...
  getById: async (req, res) => {
    try {
      const data = await IndividualizedSpecialPlan.findById(req.params.id);
  ...
  deletAll: async (req, res) => {
    try {
      const data = await IndividualizedSpecialPlan.deleteMany();
      return res.status(200).json(data);
    } catch (error) {
      return res.status(500).json({ message: error.message });
    }
  ...
  search: async (req, res) => {
    ...
    try {
      ...
      const data = await IndividualizedSpecialPlan.findOne(query);
