...
const autismSpectrumDisorderSchema = require("../../../models/v1/specialEducationService/autismSpectrumDisorder");

const Controller = {
  getAll: async (req, res) => {
    try {
      const data = await autismSpectrumDisorderSchema.find();
  ...
  createOrUpdate: async (req, res) => {
    ...
    try {
      ...
      if (id) {
        ...
        data = await autismSpectrumDisorderSchema.findByIdAndUpdate(
          id,
          req.body,
          {
            new: true,
        ...
        const core = new autismSpectrumDisorderSchema(req.body);
  ...
  getById: async (req, res) => {
    try {
      const data = await autismSpectrumDisorderSchema.findById(req.params.id);
  ...
  deletAll: async (req, res) => {
    try {
      const data = await autismSpectrumDisorderSchema.deleteMany();
      return res.status(200).json(data);
    } catch (error) {
      return res.status(500).json({ message: error.message });
    }
  ...
  search: async (req, res) => {
    ...
    try {
      ...
      const data = await autismSpectrumDisorderSchema.findOne(query);
