...
const oralReadingScreening = require('../models/oralReadingScreening')
...
const Controller = {
  ...
  create: catchAsync(async (req, res) => {
    ...
    try {
      const updateOperations = data.map(async (item) => {
        ...
        const result = await oralReadingScreening.findOneAndUpdate(
          { pupil_id, language },
          item,
  ...
  //     const NeworalReadingScreening = await oralReadingScreening.insertMany(
  //       data
  //     )
  //     if (!NeworalReadingScreening) {
  //       return res
  //         .status(400)
  //         .json({ error: 'oralReadingScreening not created ' })
  //     } else {
  //       res.status(200).json(NeworalReadingScreening)
  //     }
  //   } catch (error) {
  //     res.status(400).json({ error: error.message })
  //   }
    ...
    try {
      ...
      await oralReadingScreening
        ...
        .catch(() => {
          res
            .status(404)
            .send({ message: 'Not oralReadingScreening  with id ' + id })
        })
    } catch (error) {
      return res
        .status(400)
      ...
      oralReadingScreening
        ...
        .then((data) => {
          if (!data) {
            ...
            res.send({
              message: 'oralReadingScreening is deleted successfully!',
            })
          }
        })
        .catch((error) => {
          return res
            .status(400)
            .json({ error: `oralReadingScreening Dosent exist` })
        })
    } catch (error) {
      return res
        .status(400)
